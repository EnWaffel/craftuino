...
std::pair<int, Cmd> SetCmd::Compile(Program& prog, Func& func, const std::vector<std::string>& args)
{
    Cmd cmd{};
    cmd.name = "set";
    cmd.args = args;
...
void SetCmd::Generate(Program& prog, Func& func, Cmd& cmd, std::ofstream& out)
{
    out << cmd.args[0];
    out << '=';
    out << cmd.args[1];
