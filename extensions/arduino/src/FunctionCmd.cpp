...
std::pair<int, Cmd> FunctionCmd::Compile(Program& prog, Func& func, const std::vector<std::string>& args)
{
    Cmd cmd{};
    cmd.name = "function";
    cmd.args = args;
...
void FunctionCmd::Generate(Program& prog, Func& func, Cmd& cmd, std::ofstream& out)
{
    out << cmd.args[0] << '(';
