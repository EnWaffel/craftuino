...
std::pair<int, Cmd> GetPinCmd::Compile(Program& prog, Func& func, const std::vector<std::string>& args)
{
    Cmd cmd{};
    cmd.name = "getpin";
    cmd.args = args;
...
void GetPinCmd::Generate(Program& prog, Func& func, Cmd& cmd, std::ofstream& out)
{
    if (cmd.args.size() < 3)
    {
        out << cmd.args[1];
