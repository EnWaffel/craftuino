...
std::pair<int, Cmd> SerialCmd::Compile(Program& prog, Func& func, const std::vector<std::string>& args)
{
    Cmd cmd{};
    cmd.name = "serial";
    cmd.args = args;
...
void SerialCmd::Generate(Program& prog, Func& func, Cmd& cmd, std::ofstream& out)
{
    if (cmd.args[0] == "begin")
    {
        out << "Serial.begin(";
